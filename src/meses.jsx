...
// Chave única por mês: "2025-03"
const monthKey = (y, m) => `${y}-${String(m + 1).padStart(2, "0")}`;
const MONTHS_PT = ["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"];

function loadMonth(key) {
  try {
    return JSON.parse(localStorage.getItem(`financa_${key}`) || "null") || { fixas: [], mensais: [], income: 0 };
  } catch { return { fixas: [], mensais: [], income: 0 }; }
}
...
function ItemRow({ item, onDelete, readOnly }) {
  ...
  return (
    <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-3 hover:shadow-md transition-all duration-200">
...
function EmptyState({ icon, text }) {
  return (
    <div className="text-center py-10 px-6 text-slate-300 bg-white border-2 border-dashed border-slate-200 rounded-xl">
...
export default function App() {
  ...
  useEffect(() => {
    setMonthData(loadMonth(key));
  }, [key]);

  // Salva sempre que muda
  useEffect(() => {
    saveMonth(key, monthData);
  }, [monthData, key]);
  ...
  // Lista todos os meses com dados salvos
  function allSavedMonths() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (k?.startsWith("financa_")) {
        ...
        const total = d.fixas.reduce((s, f) => s + f.val, 0) + d.mensais.reduce((s, x) => s + x.val, 0);
        results.push({ key: mk, year: y, month: m - 1, income: d.income, total, sobrou: d.income - total });
      }
    }
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between px-7 h-16 w-full text-fuchsia-600 border-b border-fuchsia-200 bg-white sticky top-0 z-50">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`text-sm px-4 py-1.5 rounded-xl font-medium transition-all ${view === "current" ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-indigo-600"}`}
          >📋 Mês atual</button>
          <button
            onClick={() => setView("history")}
            className={`text-sm px-4 py-1.5 rounded-xl font-medium transition-all ${view === "history" ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-indigo-600"}`}
          >🗂 Histórico</button>
        </div>
      </div>
      ...
      {/* ── HISTÓRICO ── */}
      {view === "history" && (
        <div className="fade-up max-w-3xl mx-auto px-6 py-8">
          <h2 className="text-lg font-semibold text-slate-700 mb-5">🗂 Histórico por mês</h2>
          {(() => {
            const saved = allSavedMonths();
            if (saved.length === 0) return (
              <EmptyState icon="📂" text="Nenhum mês salvo ainda.<br/>Os dados aparecem aqui automaticamente." />
            );
            return (
              <div className="flex flex-col gap-3">
                {saved.map((s) => (
                  <button
                    key={s.key}
                    onClick={() => { setSelYear(s.year); setSelMonth(s.month); setView("current"); }}
                    className="flex items-center justify-between bg-white border border-slate-200 rounded-2xl px-5 py-4 hover:shadow-md hover:border-indigo-300 transition-all duration-200 text-left"
                  >
                    <div>
                      <div className="font-semibold text-slate-800 text-sm">
                        {MONTHS_PT[s.month]} {s.year}
                        {s.year === now.getFullYear() && s.month === now.getMonth() && (
                          <span className="ml-2 text-xs bg-indigo-100 text-indigo-600 rounded-full px-2 py-0.5">atual</span>
                        )}
                      </div>
      ...
      {view === "current" && (
        <div className="fade-up flex flex-col items-center p-6 gap-5 max-w-3xl mx-auto">
          ...
          {/* SELETOR DE MÊS */}
          <div className="flex items-center gap-3 bg-white w-full rounded-2xl px-6 py-4 border border-slate-100 shadow-sm">
            ...
            {!isCurrentMonth && (
              <span className="ml-auto text-xs bg-amber-100 text-amber-600 rounded-full px-3 py-1">
                👁 Visualizando histórico — somente leitura
              </span>
            )}
            {isCurrentMonth && (
          ...
          {/* RENDA */}
          <div className="flex bg-white w-full rounded-2xl items-center justify-center gap-3 px-6 py-4 border border-slate-100">
            ...
            <input
              ...
              className={`font-semibold text-base text-fuchsia-700 bg-fuchsia-50 border-2 border-fuchsia-200 rounded-xl px-5 h-11 w-48 outline-none transition-colors ${isCurrentMonth ? "focus:border-fuchsia-400" : "opacity-60 cursor-default"}`}
            />
